...
const int pwmIntervals = 10;
int old_pwm = pwmIntervals;
...
void updateBrightnessUp()
{
    if (CFG.fade_value == 0)CFG.fade_value = 1;
    
    old_pwm = CFG.fade_value;
    
    CFG.fade_value ++;

    if (CFG.fade_value > pwmIntervals) {
        CFG.fade_value = 2;
        cycleDown();
    }
    else {
...
void cycleDown()
{
    for (int interval = old_pwm; interval >= CFG.fade_value ; interval --)
    {
