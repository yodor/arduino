...
StandbyScreen::StandbyScreen() : SelectorScreen()
{
}
StandbyScreen::~StandbyScreen()
{
}
void StandbyScreen::render(char *line0, char* line1)
{
  snprintf(line0, 10, "%4s%5s", "    ", clock_str);
  snprintf(line1, 17, "%-16s", "                ");
}
void StandbyScreen::processLoop(long unsigned current_millis)
{
...
bool DisplayDevice::getPowerState()
{
  return power_state;  
}
...
void DisplayDevice::changeDisplayMode(DISPLAY_MODE mode)
{
  ...
  else if (current_mode == MODE_STANDBY) {
    
    current_screen = new StandbyScreen();  
